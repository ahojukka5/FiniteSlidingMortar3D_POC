...
from contact3d.adaptive_options import (
    AdaptiveContactOptions,
    AdaptiveLoadOptions,
    AdaptivePenaltyOptions,
)
from contact3d.adaptive_solver import contact_penalties, solve_adaptive_contact_path
...
class Mesh:
    ...
    @property
    def node_count(self): return len(self.reference_nodes)
...
class Interface:
    ...
    areas: np.ndarray
    @property
    def normal_penalty(self): return self.penalty
    def with_normal_penalty(self, normal_penalty): return replace(self, penalty=normal_penalty)
    def reference_tributary_areas(self): return self.areas.copy()
    def initial_state(self): return AugmentedLagrangeState.zeros(1)
...
class Problem:
    ...
    sparsity: object
    def initial_states(self): return tuple(i.initial_state() for i in self.interfaces)
...
def result(problem, load_factor, converged):
    ...
    evaluation = SimpleNamespace(
        contacts=contacts,
        maximum_penetration=max(c.diagnostics.maximum_penetration for c in contacts),
    ...
    return Result(
        displacement,
        tuple(AugmentedLagrangeState(np.array([load_factor])) for _ in problem.interfaces),
        converged,
...
def test_interface_local_retry_and_histories_are_transactional():
    ...
    problem=Problem(
        ...
        NeoHookeanMaterial.from_young_poisson(210., .3),
        (Interface(100., np.array([.25])), Interface(100., np.array([.01]))),
        object(),
    )
    ...
    def solver(problem, displacement, states, *, load_factor, options, tolerance):
        ...
        return result(problem, load_factor, converged=penalties[1] > 100.)

    options=AdaptiveContactOptions(
        load=AdaptiveLoadOptions(initial_step=1., minimum_step=.1, maximum_step=1.),
        penalty=AdaptivePenaltyOptions(
            increase_factor=4.,
            normalized_penetration_target=1e-6,
            maximum_scale_factor=100.,
        ),
        scaling=ScaleAwareConvergenceOptions(enabled=True, gap_tolerance=1e-6),
    )
    solved=solve_adaptive_contact_path(problem, 1., options=options, _solver=solver)
    assert solved.converged
    assert calls[0] == (100., 100.)
    assert calls[1][0] == 100.
    assert calls[1][1] > 100.
    assert contact_penalties(solved.problem)[0] == 100.
    assert solved.problem.sparsity is original_sparsity
    assert [a.action for a in solved.attempts] == ['penalty_increase', 'accepted']
